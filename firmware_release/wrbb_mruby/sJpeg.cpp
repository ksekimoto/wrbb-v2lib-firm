...
sJpeg::~sJpeg(){
    g_pInFile.close();
	DEBUG_PRINT("File closed", 0);	if (pImage != (uint8_t *)NULL) {
		free(pImage);
		pImage = (uint8_t *)NULL;
...
int sJpeg::decode(char *filename, unsigned char pReduce)
{
    ...
    DEBUG_PRINT("FileSize", g_nInFileSize);
    err = (int)pjpeg_decode_init(&image_info, (pjpeg_need_bytes_callback_t)pjpeg_need_bytes_callback, (void *)NULL, (unsigned char)reduce);
    if (err != 0) {
    ...
    decoded_width = reduce ? (image_info.m_MCUSPerRow * MCUWidth()) / 8 : width();
    decoded_height = reduce ? (image_info.m_MCUSPerCol * MCUHeight()) / 8 : height();
